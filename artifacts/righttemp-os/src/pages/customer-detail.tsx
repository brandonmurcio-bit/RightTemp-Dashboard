...
import { ChevronLeft, Save, Trash2, Calendar, MapPin, Building, Wrench, ShieldAlert } from 'lucide-react';
...
export default function CustomerDetailPage() {
  ...
  const form = useForm<CustomerUpdateValues>({
    ...
    defaultValues: {
      name: '', email: '', phone: '', address: '', status: 'active', serviceType: 'hvac_install', notes: ''
    }
  });
  ...
  const onSubmit = (data: CustomerUpdateValues) => {
    updateCustomer.mutate({ id, data }, {
      onSuccess: (updated) => {
        queryClient.setQueryData(getGetCustomerQueryKey(id), updated);
  ...
  if (isLoading) {
    return <div className="p-8 animate-pulse text-muted-foreground">Loading customer file...</div>;
  }
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => setLocation('/customers')} className="rounded-full">
            <ChevronLeft className="w-5 h-5" />
          </Button>
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{customer.name}</h1>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-sm text-muted-foreground">Customer #{customer.id}</span>
              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${customer.status === 'active' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                {customer.status}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This will permanently erase this customer and all associated history.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <Button size="sm" onClick={form.handleSubmit(onSubmit)} disabled={!form.formState.isDirty || updateCustomer.isPending}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
              ...
              <form id="customer-form" className="space-y-4">
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <div className="relative">
                    ...
                    <Textarea id="address" {...form.register('address')} className="pl-9 min-h-[80px]" />
                  </div>
                </div>
              </form>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Wrench className="w-5 h-5 text-primary" /> Equipment & Service Notes
              </CardTitle>
              <CardDescription>Document HVAC units, filter sizes, access codes, etc.</CardDescription>
            </CardHeader>
            <CardContent>
              <Textarea 
                {...form.register('notes')} 
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-5 pt-6">
              <div className="space-y-3">
                ...
                <Select onValueChange={(v) => { form.setValue('status', v as any, { shouldDirty: true }); }} value={form.watch('status')}>
                  <SelectTrigger className={form.watch('status') === 'active' ? 'border-primary/50' : ''}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-3">
                ...
                <Select onValueChange={(v) => { form.setValue('serviceType', v as any, { shouldDirty: true }); }} value={form.watch('serviceType')}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
          ...
          {form.watch('serviceType') === 'emergency' && (
            <div className="bg-destructive/10 border border-destructive/20 rounded-xl p-4 flex items-start gap-3 text-destructive">
              ...
              <div className="text-sm">
                ...
                <p className="opacity-90 mt-1">This customer requires prioritized response. Ensure rapid dispatch.</p>
              </div>
            </div>
          )}
          ...
          <div className="text-xs text-muted-foreground flex items-center justify-center gap-2 p-4">
            ...
            <span>Customer since {new Date(customer.createdAt).getFullYear()}</span>
          </div>
        </div>
      </div>
    </div>
